...
import static com.lambda.investing.algorithmic_trading.pnl_calculation.PnlSnapshot.UPDATE_HISTORICAL_LOCK;
...
public class PortfolioManager {
    ...
    public void setPortfolio(Portfolio portfolio) {
        ...
        for (String instrumentPk : portfolio.getPortfolioInstruments().keySet()) {
            ...
            pnlSnapshot.setAlgorithmInfo(this.algorithm.getAlgorithmInfo());
...
//        Long lastDepthUpdateTimestamp = this.lastDepthUpdateTimestamp.getOrDefault(key, 0L);
        ...
        if (Configuration.PORTFOLIO_MANAGER_UPDATE_FREQUENCY_MS > 0 && depth.getTimestamp() - lastPnlUpdate < Configuration.PORTFOLIO_MANAGER_UPDATE_FREQUENCY_MS) {
            pnlSnapshot.setLastDepth(depth);//only this...not recalculate
            return;
        }
    ...
    public String summary(Instrument instrument) {
        ...
        String output = String
                .format("\n\ttrades:%d (agg:%d)  position:%.3f totalPnl:%.3f totalFees:%.3f\n\trealizedPnl:%.3f  realizedFees:%.3f \n\tunrealizedPnl:%.3f  unrealizedFees:%.3f ",
                        pnlSnapshot.numberOfTrades.get(), pnlSnapshot.numberOfAggressorTrades.get(), pnlSnapshot.netPosition, pnlSnapshot.totalPnl,
    ...
    public void plotHistorical(Instrument instrument) {
        ...
        String title = String.format("%s totalPnl:%.3f  realizedPnl:%.3f  unrealizedPnl:%.3f", algorithm.getAlgorithmInfo(),
        ...
        String title2 = String
                .format("%s position:%.3f   numberOfTrades:%d (agg:%d)", algorithm.getAlgorithmInfo(), pnlSnapshot.netPosition,
                        pnlSnapshot.numberOfTrades.get(), pnlSnapshot.numberOfAggressorTrades.get());
        ...
        synchronized (UPDATE_HISTORICAL_LOCK) {
            ...
            try {
                ...
                for (String instrumentPk : instrumentPnlSnapshotMap.keySet()) {
                    ...
                    Table output1 = Table.create(algorithm.getAlgorithmInfo());
                    output1 = output1
