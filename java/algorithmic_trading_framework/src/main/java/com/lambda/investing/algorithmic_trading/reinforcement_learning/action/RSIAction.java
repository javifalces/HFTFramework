...
public class RSIAction extends AbstractAction {
	...
	public static int SIZE_ARRAY_ACTION = 6;//0-4
	...
	public RSIAction(int[] periods, double[] upperBounds, double[] upperBoundsExits, double[] lowerBounds,
			double[] lowerBoundsExits, int[] changeSides
		...
		if (periods.length == 0) {
			...
			validInputsSize.add(periods.length);
		}
		...
		if (upperBounds.length == 0) {
			...
			validInputsSize.add(upperBounds.length);
		}
		this.upperBounds = upperBounds;

		if (upperBoundsExits.length == 0) {
			upperBoundsExits = null;
		} else {
			validInputsSize.add(upperBoundsExits.length);
		}
		...
		if (lowerBounds.length == 0) {
			...
			validInputsSize.add(lowerBounds.length);
		}
		this.lowerBounds = lowerBounds;

		if (lowerBoundsExits.length == 0) {
			lowerBoundsExits = null;
		} else {
			validInputsSize.add(lowerBoundsExits.length);
		}
	...
	private void fillCacheActions() {
		...
		System.out.println("RSIAction has " + String.valueOf(actionIndexToArr.size()) + " actions");
	...
	private class ActionRow {
		...
		@Override public int hashCode() {
			...
			return Objects.hash(period, upperBound, upperBoundExit, lowerBound, lowerBoundExit, changeSide);
		}
