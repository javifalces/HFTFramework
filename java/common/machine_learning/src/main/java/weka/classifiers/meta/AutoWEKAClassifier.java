...
public class AutoWEKAClassifier extends AbstractClassifier implements AdditionalMeasureProducer {
	...
	static enum Resampling {
		CrossValidation, MultiLevel, RandomSubSampling, TerminationHoldout
	}
	 ...
	 * Metrics to maximise.
	 */
	static final Metric[] metricsToMax = { Metric.areaUnderROC, Metric.correct, Metric.correlationCoefficient,
			Metric.fMeasure, Metric.kappa, Metric.kBInformation, Metric.kBMeanInformation, Metric.kBRelativeInformation,
			Metric.pctCorrect, Metric.precision, Metric.weightedAreaUnderROC, Metric.weightedFMeasure,
			Metric.weightedPrecision, Metric.weightedRecall, Metric.weightedTrueNegativeRate,
	...
	static {
		...
		resamplingArgsMap
				.put(Resampling.MultiLevel, "numLevels=2[$]autoweka.instancegenerators.CrossValidation[$]numFolds=10");
	...
	public static final String configurationRankingPath =
			"ConfigurationLogging" + File.separator + "configuration_ranking.xml";
	/**
	 * The path for the log with the hashcodes for the configs we have
	 **/
	public static final String configurationHashSetPath =
			"ConfigurationLogging" + File.separator + "configuration_hashes.txt";
	/**
	...
	public AutoWEKAClassifier() {
		...
		props.setProperty("javax.xml.parsers.DocumentBuilderFactory",
				"com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl");
		props.setProperty("javax.xml.parsers.SAXParserFactory",
	...
	public void buildClassifier(Instances is) throws Exception {
				...
				public void run() {
					...
					try {
						...
						Pattern p = Pattern.compile(
								".*Estimated mean quality of final incumbent config .* on test set: (-?[0-9.]+).*");
						...
						while ((line = reader.readLine()) != null) {
							...
							if (line.matches(".*DEBUG.*") || line.matches(".*Variance is less than.*")) {
								...
								if (line.matches(".*ClassifierRunner - weka.classifiers.*")) {
									...
									if (wLog != null) {
										String msg =
												"Thread " + index + ": performed " + tried + " evaluations, estimated "
														+ metric + " " + bestMetricValue + "...";
		...
		if (allFailed) {
			throw new Exception(
					"All runs timed out, unable to find good configuration. Please allow more time and rerun.");
		}
		...
		if (nBestConfigs > 1) {
			try {
				ConfigurationRanker
						.rank(nBestConfigs, msExperimentPaths[bestIndex] + expName + File.separator, mBest.rawArgs);
				bestConfigsCollection = ConfigurationCollection
						.fromXML(msExperimentPaths[bestIndex] + expName + File.separator + configurationRankingPath,
								ConfigurationCollection.class);
			} catch (Exception e) {
				if (e instanceof FileNotFoundException || e instanceof NoSuchElementException) {
					System.out.println(
							"Could not find any configuration fully evaluated on all 10 folds. Please provide Auto-WEKA with more run time");
		...
		log.info(
				"classifier: {}, arguments: {}, attribute search: {}, attribute search arguments: {}, attribute evaluation: {}, attribute evaluation arguments: {}",
				classifierClass, classifierArgs, attributeSearchClass, attributeSearchArgs, attributeEvalClass,
				attributeEvalArgs);
	...
	public TechnicalInformation getTechnicalInformation() {
		...
		result.setValue(Field.TITLE,
				"Auto-WEKA: Combined Selection and Hyperparameter Optimization of Classifiaction Algorithms");
	...
	public String toString() {
		String res = "best classifier: " + classifierClass + "\n" + "arguments: " + (classifierArgs != null ?
				Arrays.toString(classifierArgs) :
				"[]") + "\n" + "attribute search: " + attributeSearchClass + "\n" + "attribute search arguments: " + (
				attributeSearchArgs != null ?
						Arrays.toString(attributeSearchArgs) :
						"[]") + "\n" + "attribute evaluation: " + attributeEvalClass + "\n"
				+ "attribute evaluation arguments: " + (attributeEvalArgs != null ?
				Arrays.toString(attributeEvalArgs) :
		...
		if (nBestConfigs > 1) {
			...
			if (bestConfigsCollection == null) {
				...
				res += "\nPlease note that this list only contains configurations evaluated on at least "
						+ maxFoldEvaluationAmt + " folds,";
		...
		if (totalTried < 1000) {
			res += "Tried " + totalTried
					+ " configurations; to get good results reliably you may need to allow for trying thousands of configurations.\n";
		}
		return res;
	}
