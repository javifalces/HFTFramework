...
@XmlRootElement(name = "listsexperiment") @XmlAccessorType(XmlAccessType.NONE) class ListExperiment
	...
	@XmlElement(name = "argstrings") public ArrayList<String> argStrings = new ArrayList<String>();
	@XmlElement(name = "seed") public String seed = "0";
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-target")) {
				...
				experiment = ListExperiment.fromXML(
						URLDecoder.decode(experimentDir.getAbsolutePath()) + File.separator + experimentDir.getName()
								+ ".listexperiment");
			}
		}
		...
		for (String s : InstanceGenerator.create(experiment.instanceGenerator, "__dummy__")
				.getAllInstanceStrings(experiment.instanceGeneratorArgs))
			instanceStrings.add(s);
		outputFileName = URLDecoder.decode(experimentDir.getAbsolutePath()) + File.separator + experimentDir.getName()
				+ ".listresults";
		...
		for (String argString : argStrings) {
			...
			for (String instanceString : instanceStrings) {
				...
				res.results.add(new ListResultGroup.ListResult.InstanceResult(instanceString, SubProcessWrapper
						.getErrorAndTime(experimentDir, experiment, instanceString, argString, experiment.seed)));
			}
		}
