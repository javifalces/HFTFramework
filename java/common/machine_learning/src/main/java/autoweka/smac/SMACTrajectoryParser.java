...
public class SMACTrajectoryParser extends TrajectoryParser {
	...
	private Pattern mTrajPattern = Pattern
			.compile("([\\-\\.\\d]+), ([\\-\\.\\dEef]+), [\\-\\.\\d]+, [\\-\\.\\d]+, [\\-\\.\\d]+, (.*)");
	...
	public SMACTrajectoryParser() {
		super();
	}

	;//To help with the unit tests, don't remove.
	...
	public Trajectory parseTrajectory(Experiment experiment, File folder, String seed) {
		...
		ClassParams params = new ClassParams(
				URLDecoder.decode(folder.getAbsolutePath()) + File.separator + "autoweka.params");
		...
		try {
			...
			String trajFileName = "";
			File[] files = new File(
					URLDecoder.decode(folder.getAbsolutePath()) + File.separator + "out" + File.separator + "autoweka")
					.listFiles();
			for (File f : files) {
				String s = f.getName();
			...
			files = new File(
					URLDecoder.decode(folder.getAbsolutePath()) + File.separator + "out" + File.separator + "autoweka"
							+ File.separator + "state-run" + seed + File.separator).listFiles();
			for (File f : files) {
				String s = f.getName();
