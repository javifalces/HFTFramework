...
public class SMACExperimentConstructor extends ExperimentConstructor {
	...
	@Override public List<String> getCallString(String experimentPath) {
		// assumes that autoweka.jar is at the root of the autoweka distribution
		// (as it will be for the WEKA package)
		String prefix = new File(URLDecoder
				.decode(SMACExperimentConstructor.class.getProtectionDomain().getCodeSource().getLocation().getPath()))
		...
		args.add(experimentPath + File.separator + "out" + File.separator + "runstamps" + File.separator
				+ "{SEED}.stamp");
	...
	public void printFeatureFile(PrintStream out) {
		...
		Map<String, Map<String, String>> features = mInstanceGenerator
				.getAllInstanceFeatures(mExperiment.instanceGeneratorArgs);
	...
	public void printScenarioFile(PrintStream out) {
		...
		if (!mInstanceGenerator.getAllInstanceFeatures(mExperiment.instanceGeneratorArgs).isEmpty()
				&& mProperties.get("instancesOverride") != null)
