...
@XmlRootElement(name = "configuration") public class Configuration extends XmlSerializable implements Comparable {

	@XmlElement(name = "argStrings") private String mArgStrings;

	@XmlElement(name = "evaluatedScore") private double mEvaluatedScore;

	@XmlElement(name = "evaluatedFold") private int mEvaluatedFold;

	@XmlElement(name = "folds") private ArrayList<String> mFolds;

	@XmlElement(name = "scores") private ArrayList<String> mScores;

	@XmlElement(name = "avgScore") private double mAverageScore;

	@XmlElement(name = "amtScores") private int mAmtScores;

	@XmlElement(name = "amtFolds") private int mAmtFolds;

	@XmlElement(name = "avgFlag") private boolean averagedFlag;
	...
	*/
	...
	private double average(
			List<String> l) { //Apparently theres no standard java method for that. @TODO Check if thats true
		double sum = 0;
		for (String d : l) {
	...
	public int compareTo(
			Object aTarget) { //Compares only the average score. If necessary, updates this metric before comparing
		...
		if (this.mFolds.size() > cTarget.mFolds.size()) {
			...
			if (this.mAverageScore < cTarget.mAverageScore)
				return 1; //Assumes smaller score is better. If that isn't the case, change that.
			else if (this.mAverageScore > cTarget.mAverageScore)
				return -1; // @TODO make this class receive a Metric as input and do this change automatically
			else
				return 0;
		}
	...
	*/
	...
	public double getEvaluatedScore() {
		return mEvaluatedScore;
	}

	public int getEvaluatedFold() {
		return mEvaluatedFold;
	}
