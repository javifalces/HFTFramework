...
public class TrajectoryPointPredictionRunner {
	...
	public static void main(String[] args) {
		...
		for (Trajectory traj : trajGroup.getTrajectories()) {
			...
			props.put("predictionsFileName",
					URLDecoder.decode(experimentDir.getAbsolutePath()) + "/predictions." + traj.getSeed() + ".csv");
			if (saveModel)
				props.put("modelOutputFilePrefix",
						URLDecoder.decode(experimentDir.getAbsolutePath()) + "/trained." + traj.getSeed());
			SubProcessWrapper
					.getErrorAndTime(experimentDir, experiment, instance, point.getArgs(), traj.getSeed(), props);
		}
	}
}
