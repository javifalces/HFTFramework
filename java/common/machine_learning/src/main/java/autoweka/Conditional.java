...
public class Conditional {
	...
	public Conditional(String line, HashMap<String, Parameter> paramMap) {
		...
		if (parameter == null)
			throw new IllegalArgumentException(
					"Can't find conditional target parameter with name '" + m.group(1) + "'");
		if (parent == null)
			throw new IllegalArgumentException(
					"Can't find conditional parent parameter with name '" + m.group(2) + "'");
	...
	private static final Pattern conditionalPattern = Pattern
			.compile("\\s*([a-zA-Z0-9_-]*)\\s*\\|\\s*([a-zA-Z0-9_]*)\\s*in\\s*\\{(.*)\\}.*");
}
