...
*/
@XmlRootElement(name = "configurationCollection") public class ConfigurationCollection extends XmlSerializable {

	@XmlElement(name = "configurations") private ArrayList<Configuration> mConfigurations;
	...
	public int getFullyEvaluatedAmt() {
		...
		for (Configuration c : mConfigurations) {
			if (c.getAmtFolds() == largestFoldAmt)
				counter++;
		}
		return counter;
	}
	...
	public static <T extends XmlSerializable> T fromXML(String filename,
			Class<T> c) { //Original is protected so we're overriding to make it public.
