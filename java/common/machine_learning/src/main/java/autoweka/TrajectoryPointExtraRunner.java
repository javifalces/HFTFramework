			...
			for (Trajectory.Point point : points) {
				...
				for (Experiment.TrajectoryPointExtra extra : experiment.trajectoryPointExtras) {
					...
					if (mResultCache.get(cacheLookup) != null) {
						...
						errTime = SubProcessWrapper
								.getErrorAndTime(experimentDir, experiment, extra.instance, point.getArgs(),
										traj.getSeed(), runnerProps);
					}
					mResultCache.put(cacheLookup, errTime);

					point.specificInstanceInfo
							.add(new Trajectory.Point.SpecificInstanceInfo(extra.name, extra.instance, errTime.error,
									errTime.time));
				}
