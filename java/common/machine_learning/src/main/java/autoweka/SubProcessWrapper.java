...
public class SubProcessWrapper extends Wrapper {
	...
	private static Pattern mResultPattern = Pattern
			.compile("SubProcessWrapper: Time\\(([\\.\\d]+)\\) Score\\(([\\.\\deE+-]+)\\)");
	 ...
	 */
	@Override protected void _processParameter(String arg, Queue<String> args) {
		if (arg.equals("-seed")) {
			// ignored
			args.poll();
	 ...
	 */
	@Override protected void _processWrapperParameterStart(Queue<String> args) {
	 ...
	 */
	@Override protected void _processResults(ClassifierResult res) {
			...
			try {
				if (res.getAttributeSelection() != null) {
					weka.core.SerializationHelper
							.write(outputFilePrefix + ".attributeselection", res.getAttributeSelection());
				} else {
	 ...
	 */
	public static ErrorAndTime getErrorAndTime(File runDir, ListExperiment exp, String instance, String args,
			String autowekaSeed) {
		return getErrorAndTime(runDir, exp, instance, args, autowekaSeed, new Properties());
	}

	public static ErrorAndTime getErrorAndTime(File runDir, ListExperiment exp, String instance, String args,
			String autowekaSeed, Properties props) {
	 ...
	 */
	public static ErrorAndTime getErrorAndTime(File runDir, Experiment exp, String instance, String args,
			String autowekaSeed) {
		return getErrorAndTime(runDir, exp, instance, args, autowekaSeed, new Properties());
	}
	 ...
	 */
	public static ErrorAndTime getErrorAndTime(File runDir, String memory, Properties props, float trainTimeout,
