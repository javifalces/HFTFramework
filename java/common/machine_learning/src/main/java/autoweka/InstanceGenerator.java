...
public abstract class InstanceGenerator {
	...
	public InstanceGenerator(String datasetString) {
		if (datasetString.equals("__dummy__")) {
			...
			try {
				...
				log.warn(
						"It looks like you're using an old experiment that doesn't indicate the type of dataset that it is using");
			...
			if (type == null) {
				...
				loadTrainTestArff(props.getProperty("trainArff"), props.getProperty("testArff"),
						props.getProperty("classIndex", "last"));
			} else {
	...
	static public InstanceGenerator create(String className, Instances training, Instances testing) {
		...
		try {
			className = className.trim();
			cls = Class.forName(className);
			return (InstanceGenerator) cls.getDeclaredConstructor(Instances.class, Instances.class)
					.newInstance(training, testing);
		} catch (ClassNotFoundException e) {
			throw new RuntimeException("Could not find class '" + className + "': " + e, e);
		} catch (Exception e) {
			throw new RuntimeException("Failed to instantiate '" + className + "': " + e, e);
