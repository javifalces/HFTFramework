...
abstract class WorkerThread extends Thread {
	...
	private com.sun.management.OperatingSystemMXBean mOSBean = (com.sun.management.OperatingSystemMXBean) java.lang.management.ManagementFactory
	...
	float runWorker(float timeoutSeconds) {
		...
		while (true) {
			...
			if (!interrupted && (mOSBean.getProcessCpuTime() - startTime
					> timeout /*|| wallTime > timeout * mWalltimeMultiplyer*/)) {
				...
				interrupted = true;
			} else if (!stopped && (mOSBean.getProcessCpuTime() - startTime > timeout
					* msTimeoutMultiplyer /*|| wallTime > timeout * mWalltimeMultiplyer * mTimeoutMultiplyer*/)) {
				//Try to interrupt the bugger
				this.terminate();
