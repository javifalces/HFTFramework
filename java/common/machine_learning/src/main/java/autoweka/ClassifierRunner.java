...
public class ClassifierRunner {
	...
	public ClassifierRunner(Properties props) {
		...
		mInstanceGenerator = InstanceGenerator
				.create(props.getProperty("instanceGenerator"), props.getProperty("datasetString"));
	...
	private class RunnerThread extends WorkerThread {
		...
		public RunnerThread(String _instanceStr, String _resultMetric, float _timeout, String _mSeed,
				List<String> _args) {
	 ...
	 */
	public ClassifierResult run(String instanceStr, String resultMetric, float timeout, String mSeed,
			List<String> args) {
	 ...
	 */
	private ClassifierResult _run(String instanceStr, String resultMetric, float timeout, String mSeed,
			List<String> args) {
		...
		if (!mTestOnly && ((attribSearchClassName != null && !attribSearchClassName.equals("NONE")) || (
				attribEvalClassName != null /*&& !attribEvalClassName.equals("NONE")*/))) {
			...
			try {
				...
				throw new RuntimeException(
						"Failed to create ASEvaluation " + attribEvalClassName + ": " + e.getMessage(), e);
			}
			try {
				asSearch = ASSearch
						.forName(attribSearchClassName, argMap.get("attributesearch").toArray(new String[0]));
			} catch (Exception e) {
				throw new RuntimeException("Failed to create ASSearch " + attribSearchClassName + ": " + e.getMessage(),
						e);
			}
			...
			if (asThread.getException() != null || asThread.terminated()) {
				if (asThread.getException() != null) {
					...
					log.warn("Attribute selection (search {} {}, eval {} {}) failed: {}", attribSearchClassName,
							argMap.get("attributeeval"), attribEvalClassName, argMap.get("attributesearch"),
							asThread.getException().getMessage(), asThread.getException());
				}
		...
		try {
			...
			throw new RuntimeException("Failed to instantiate '" + targetClassifierName + "' with options " + Arrays
					.toString(argsArraySaved) + ": " + e.getMessage(), e);
		}
        /*
        try
        {
		...
		if (builderThread.getException() != null) {
			log.warn("Training classifier ({} {}) failed: {}", targetClassifierName, argsArraySaved,
					builderThread.getException().getMessage(), builderThread.getException());
		...
		log.info("{};{};{};{};{};{};{};{}", targetClassifierName, argsArraySaved, attribEvalClassName,
				argMap.get("attributeeval"), attribSearchClassName, argMap.get("attributesearch"), instanceStr,
				res.getRawScore());
	...
	private boolean _evaluateClassifierOnInstances(AbstractClassifier classifier, ClassifierResult res,
		...
		try {
			...
			log.debug("Completed evaluation on {}/{} instances.", (instances.numInstances() - eval.unclassified()),
	...
	protected void saveConfiguration(ClassifierResult res, List<String> args, String instanceStr) {
		...
		if (ciFile.exists()) {
			Configuration ciConfigFull = Configuration
					.fromXML(ciFilename, Configuration.class); //Find a faster way w/o IOs?
