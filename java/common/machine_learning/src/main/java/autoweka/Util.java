...
public class Util {
	...
	static public void parsePropertyString(Properties props, String propStr) {
		...
		for (String str : strArray) {
			...
			//String value = str.substring(equalsIndex+1).replace("[@]:", ":");
			String value = str.substring(equalsIndex + 1).replace("[@]:", ":").replace("\\:", ":").replace("[]:", ":")
					.replace("__COLONESCAPE__:", ":");
			...
			try {
				...
				for (int i = 0; i < splitVar.length; i++) {
					Process shellExec = Runtime.getRuntime()
							.exec(new String[] { "cmd.exe", "/C", "echo", splitVar[i] });

					java.io.BufferedReader reader = new java.io.BufferedReader(
							new java.io.InputStreamReader(shellExec.getInputStream()));
	...
	static public List<String> splitNestedString(String params, String delimStr, int numLeft, int numRight) {
		...
		for (int i = 0; i < numLeft; i++) {
			...
			if (endIndex == -1)
				throw new IllegalArgumentException(
						"Could only find " + i + " left args before not finding the delimiter");
		...
		for (int i = 0; i < numRight; i++) {
			...
			if (startIndex == -1)
				throw new IllegalArgumentException(
						"Could only find " + i + " right args before not finding the delimeter");
		...
		if (middleStart >= middleEnd) {
			throw new IllegalArgumentException(
					"The number of left and right args doesn't leave anything in the middle");
		}
	 ...
	 */
	static public Instances createDummyInstances(int numInstances, int numClasses, int numUsefulNumeric,
			int numUsefulCategorical, int numRandomNumeric, int numRandomCategorical, int randomCategoricalSize,
			int seed) {
	...
	public static String getAutoWekaDistributionPath() {
		String locStr = URLDecoder.decode(Util.class.getClassLoader()
				.getResource(Util.class.getCanonicalName().replaceAll("\\.", "/") + ".class").toString());
		...
		if (!msFailedToFindDistributionOnce) {
			log.warn(
					"Could not auto-detect the location of your Auto-WEKA install - have you moved the classes away from the 'params' diectory?");
