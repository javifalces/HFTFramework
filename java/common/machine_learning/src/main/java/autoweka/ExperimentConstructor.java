...
public abstract class ExperimentConstructor {
	...
	private void run(Experiment exp, List<String> args) {
		...
		String absExperimentDir = URLDecoder
				.decode(new File(mExperimentPath + File.separator + mExperiment.name + File.separator)
		...
		mExperiment.toXML(mExperimentPath + File.separator + mExperiment.name + File.separator + mExperiment.name
				+ ".experiment");
	}
			...
			for (ClassParams param : params) {
				...
				if (prefixes.contains(prefix)) {
					throw new RuntimeException(
							"Prefix '" + prefix + "' (" + param.getTargetClass() + ") is already in use.");
				}
				prefixes.add(prefix);
			}
		}
