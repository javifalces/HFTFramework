...
@XmlRootElement(name = "experiment") @XmlAccessorType(XmlAccessType.NONE) public class Experiment
	 ...
	 */
	@XmlElement(name = "resultMetric") public String resultMetric = "errorRate";
	/**
	 ...
	 */
	@XmlElement(name = "datasetString") public String datasetString;
	/**
	 ...
	 */
	@XmlElement(name = "trajectoryParserClass") public String trajectoryParserClassName;
	/**
	 * The list of command line arguments that are used to invoke the SMBO method - this is set by the ExperimentConstructor
	 */
	@XmlElement(name = "callString") public List<String> callString = new ArrayList<String>();
	/**
	 ...
	 */
	@XmlElement(name = "envVariables") public List<String> envVariables = new ArrayList<String>();
	/**
	 ...
	 */
	@XmlElement(name = "extraProps") public String extraPropsString;
	/**
	 ...
	 */
	@XmlElement(name = "trajectoryPointExtras") public List<TrajectoryPointExtra> trajectoryPointExtras = new ArrayList<TrajectoryPointExtra>();

	/**
	 * Class capturing any of the extra dataset partitions that should be run on all the trajectory points for analysis
	 */
	@XmlRootElement(name = "trajpointextra") @XmlAccessorType(XmlAccessType.NONE) static public class TrajectoryPointExtra {
		 ...
		 */
		@XmlElement(name = "instance") public String instance;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-noexit"))
				...
				experiment = new File(
						URLDecoder.decode(expFolder.getAbsolutePath()) + File.separator + expFolder.getName()
								+ ".experiment");
			} else if (seed == null)
				seed = args[i];
			else
		...
		try {
			...
			File stampFile = new File(
					experiment.getParentFile() + File.separator + "out" + File.separator + "runstamps" + File.separator
							+ seed + ".stamp");
			...
			BufferedWriter logOutput = new BufferedWriter(new FileWriter(
					experiment.getParentFile() + File.separator + "out" + File.separator + "logs" + File.separator
							+ seed + ".log"));
	...
	public static Experiment createFromFolder(File folder) {
		File experiment = new File(
				URLDecoder.decode(folder.getAbsolutePath()) + File.separator + folder.getName() + ".experiment");
		try {
