...
public class MetatraderTradingEngine extends AbstractBrokerTradingEngine {
    ...
    protected Queue<String> completelyFilledOrdersNotified;
    ...
    public MetatraderTradingEngine(ConnectorConfiguration orderRequestConnectorConfiguration,
        ...
        completelyFilledOrdersNotified = EvictingQueue.create(DEFAULT_QUEUE_SIZE);
    ...
    public boolean closeTrade(OrderRequest orderRequest) {
        ...
        closeOrder.setAction(MtAction.POS_CLOSE_PARTIAL);
    ...
    public boolean orderRequest(OrderRequest orderRequest) {
        ...
        if (orderRequest.getQuantity() <= 0.0) {
            logger.error("orderRequest {} qty is negative/zero {} -> return false", orderRequest.getClientOrderId(), orderRequest.getQuantity());
            return false;
        }
        ...
        System.out.println(Configuration.formatLog("orderRequest : {} ", orderRequest));
        if (nettingByEngine) {
            //check position
...
//        logger.debug("onOpenPositions Metatrader  {}", content);
        ...
        try {
            mtPosition = fromJsonStringGSON(content, MtPosition.class);
        } catch (Exception e) {
...
//            logger.warn("no positions received from MT");
            ...
            for (String instrumentPK : lastPosition.keySet()) {
                double prevInstrumentPosition = lastPosition.get(instrumentPK);
                logger.info("clean onOpenPositions {} {} -> {}", instrumentPK, prevInstrumentPosition, 0.0);
            }

            lastPosition.clear();
            return;
        }
        Map<String, Double> prevPositionMap = new HashMap<>(lastPosition);
        lastPosition.clear();
        for (String id : mtPosition.get_positions().keySet()) {
            ...
            double otherTradesPosition = lastPosition.getOrDefault(instrumentPK, 0.0);
            double newPosition = otherTradesPosition + lots;
            lastPosition.put(instrumentPK, newPosition);
        }

        for (String instrumentPK : lastPosition.keySet()) {
            double newPosition = lastPosition.getOrDefault(instrumentPK, 0.0);
            double prevPosition = prevPositionMap.getOrDefault(instrumentPK, 0.0);
            boolean isDifferentPosition = Math.abs(newPosition - prevPosition) > 1E-6;
            if (isDifferentPosition) {
                logger.info("update onOpenPositions {} {} -> {}", instrumentPK, prevPosition, newPosition);
            }
        }
        //get key in prevPositionMap that are not in lastPosition
        Set<String> keysToRemove = new HashSet<>(prevPositionMap.keySet());
        keysToRemove.removeAll(lastPosition.keySet());
        for (String instrumentPK : keysToRemove) {
            double prevPosition = prevPositionMap.getOrDefault(instrumentPK, 0.0);
            if (Math.abs(prevPosition) > 1E-6) {
                logger.info("update onOpenPositions {} {} -> {}", instrumentPK, prevPosition, 0.0);
            }
        }
    ...
    private Pair<Boolean, ExecutionReport> notifyActiveIfSuccess(OrderRequest pendingOrderRequest, MTMessageER mtMessageER) {
        ...
        if (completelyFilledOrdersNotified.contains(pendingOrderRequest.getClientOrderId())) {
...
//            }
    ...
    private boolean isCloseTrade(MTMessageER mtMessageER) {
        ...
        boolean isCloseMessage = mtMessageER.get_action().equalsIgnoreCase("CLOSE");
        boolean isCloseTrade = isCloseOrderTrade || isPartiallyClosePosition || isCloseMessage;
        return isCloseTrade;
    }
    ...
    private void treatRejection(MTMessageER mtMessageER) {
        ...
        if (completelyFilledOrdersNotified.contains(executionReport.getClientOrderId())) {
            logger.warn("ER warning discard notify Rejection if cf already processed {} : {} ",
                    executionReport.getInstrument(), executionReport.getClientOrderId());
            return;
        }
        System.out.println(Configuration.formatLog("notifyExecutionReportById : {} ", executionReport));
        notifyExecutionReportById(executionReport);
    ...
    private void treatConfirmation(MTMessageER mtMessageER) {
        ...
        if (executionReport.getExecutionReportStatus() == ExecutionReportStatus.Active && activeOrders.containsKey(pendingOrderRequest.getClientOrderId())) {
            logger.warn("activeOrders: active already contains {} -> not notify it", pendingOrderRequest.getClientOrderId());
            return;
        }
    ...
    private void treatTrade(MTMessageER mtMessageER) {
        ...
        if (activeOrderRequest == null) {
            logger.warn("activeOrderRequest not found for clOrdId {} -> create confirmation and continue", clOrdId);
            treatConfirmation(mtMessageER);

            ticket = mtMessageER.get_ticket();
            clOrdId = MTCommand.getClientOrderId(ticket);
            if (clOrdId == null) {
                //after closing position + trade
                logger.error("{} not found clOrdId 2 -> not processed {}", ticket, mtMessageER);
                try {
                    Thread.sleep(100);
                } catch (InterruptedException e) {
                    e.printStackTrace();
        ...
        if (!isCF) {
            ...
            if (completelyFilledOrdersNotified.contains(executionReportExecuted.getClientOrderId())) {
            ...
            if (symbol != null) {
                ...
                for (OrderRequest orderRequest : LastBuyOrSell) {
                    if (orderRequest.getInstrument().equals(instrumentPK)) {
                        if (completelyFilledOrdersNotified.contains(orderRequest.getClientOrderId())) {
...
//            mtMessageER = fromJsonString(content, MTMessageER.class);//not working very good
            mtMessageER = fromJsonStringGSON(content, MTMessageER.class);
        } catch (Exception e) {
        ...
        logger.info("onUpdateExecutionReport Metatrader  {} {}", typeMessage, content);
        String magic = mtMessageER.get_magic();
        String ticket = mtMessageER.get_ticket();
