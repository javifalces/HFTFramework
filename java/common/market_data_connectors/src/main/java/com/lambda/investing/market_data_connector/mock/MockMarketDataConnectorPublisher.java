...
public class MockMarketDataConnectorPublisher extends AbstractMarketDataConnectorPublisher implements Runnable {
	...
	public MockMarketDataConnectorPublisher(ConnectorConfiguration connectorConfiguration,
			ConnectorPublisher connectorPublisher, long sleepUpdatesMs) {
	...
	private Depth createDepth(MockMarketDataConfiguration mockMarketDataConfiguration) {
		...
		for (int levelIterator = 0; levelIterator < mockMarketDataConfiguration.getLevels(); levelIterator++) {
			...
			askQuantity[levelIterator] =
					midQuantity + (levelIterator + 1) * (mockMarketDataConfiguration.getDelta() * random.nextDouble());
	...
	private Trade modifyTrade(MockMarketDataConfiguration mockMarketDataConfiguration, Trade lastTrade) {
		...
		double price = lastMidPrice
				.getOrDefault(mockMarketDataConfiguration, mockMarketDataConfiguration.getStartMidPrice());
	...
	private Depth modifyDepth(MockMarketDataConfiguration mockMarketDataConfiguration, Depth lastDepth) {
		...
		double midQuantity =
				mockMarketDataConfiguration.getStartMidQuantity() * (1 + ((random.nextDouble() - 0.5)) / 10);
	...
	@Override public void run() {
		...
		for (MockMarketDataConfiguration mockMarketDataConfiguration : lastDepthSent.keySet()) {
			logger.info("Starting publishing {}   in tcp://{}:{}",
					mockMarketDataConfiguration.getInstrument().getPrimaryKey(), zeroMqConfiguration.getHost(),
			...
			if (enable) {
				...
				for (MockMarketDataConfiguration mockMarketDataConfiguration : lastDepthSent.keySet()) {
					if (this.random.nextDouble() > mockMarketDataConfiguration.getProbabilityTrade()) {
						...
						Depth lastDepth = lastDepthSent
								.getOrDefault(mockMarketDataConfiguration, createDepth(mockMarketDataConfiguration));
						...
						//create last trade
						Trade lastTrade = lastTradeSent
								.getOrDefault(mockMarketDataConfiguration, createTrade(mockMarketDataConfiguration));
