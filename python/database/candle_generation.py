...
def generate_candle_time(df, resolution='MIN',num_units=1)->pd.DataFrame:
    ...
    :return:
    '''
    output_df = mlfinlab.data_structures.get_time_bars(file_path_or_df=df.reset_index(),resolution=resolution,num_units=num_units)
...
def generate_candle_tick(df, number_of_ticks=5)->pd.DataFrame:
    output_df = mlfinlab.data_structures.get_tick_bars(file_path_or_df=df.reset_index(),threshold=number_of_ticks)
    output_df.set_index('date_time',inplace=True)
    return output_df

def generate_candle_volume(df, volume=5000):
    output_df = mlfinlab.data_structures.get_volume_bars(file_path_or_df=df.reset_index(),threshold=volume)
    output_df.set_index('date_time',inplace=True)
    return output_df

def generate_candle_dollar_value(df, dollar_value=5000):
    output_df = mlfinlab.data_structures.get_dollar_bars(file_path_or_df=df.reset_index(),threshold=dollar_value)
    output_df.set_index('date_time',inplace=True)
    return output_df
