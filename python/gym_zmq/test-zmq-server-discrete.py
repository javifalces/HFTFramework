...
if __name__ == '__main__':
    ...
    action_columns = 8
    port = 666
    print(rf"actions(PYTHON) [{action_columns}] : discrete ")
    print(rf"states(PYTHON) [{state_columns}]  ")
    print(rf"PORT: {port}")
    # min value value
    low = list_value(
    ...
    # low = list_value(value=float('-inf'), size=action_columns)  # [-1.0, -1.0, -1.0, -1.0]
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            try:
                ...
                print(rf"env.step({action}) failed with {e}")
            if "message" in info:
                try:
                    message = info["message"]
                    if "ERROR" in message:
                        print(rf"failed sending action {action} with {message}")
                        time.sleep(5)
                except Exception as e:
                    print(rf"strange exception {e}")
