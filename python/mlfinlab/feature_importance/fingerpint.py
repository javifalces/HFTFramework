...
class AbstractModelFingerprint(ABC):
    ...
    def __init__(self):
        ...
        self.ind_partial_dep_functions = (
            None  # partial dependence function values containers
        )
        self.feature_column_position_mapping = (
            None  # numerical mapping between columns string and its position
        )
    ...
    def fit(
        ...
        if pairwise_combinations is not None:
            pairwise_effect = self._get_pairwise_effect(
                pairwise_combinations, model, X, num_values
            )
            self.pair_wise_effect = {
                'raw': pairwise_effect,
                'norm': self._normalize(pairwise_effect),
            }

        # Save results
        self.linear_effect = {
            'raw': linear_effect,
            'norm': self._normalize(linear_effect),
        }
        self.non_linear_effect = {
            'raw': non_linear_effect,
            'norm': self._normalize(non_linear_effect),
        }
    ...
    def _get_feature_values(self, X: pd.DataFrame, num_values: int) -> None:
        ...
        self.feature_values = (
            {}
        )  # Dictionary of feature values range used to build dependence functions
        ...
        self.ind_partial_dep_functions = pd.DataFrame(
            index=list(range(num_values)), columns=X.columns
        )

        self.feature_column_position_mapping = dict(
            zip(X.columns, range(0, X.shape[1]))
        )

    def _get_individual_partial_dependence(
    ...
    def _get_pairwise_effect(
        self, pairwise_combinations: list, model: object, X: pd.DataFrame, num_values
    ) -> dict:
        """
        ...
        for pair in pairwise_combinations:
            function_values = (
                []
            )  # Array of pairwise interactions [f_k_l(not centered), f_k(centered), f_l(centered)]
            col_k = pair[0]
            col_l = pair[1]

            y_cdf_k_centered = self.ind_partial_dep_functions[col_k] - np.mean(
                self.ind_partial_dep_functions[col_k]
            )
            y_cdf_l_centered = self.ind_partial_dep_functions[col_l] - np.mean(
                self.ind_partial_dep_functions[col_l]
            )
            ...
            function_values = np.array(
                function_values
            )  # Convert to np.array to vectorize operations
