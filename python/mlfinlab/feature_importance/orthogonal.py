...
def _get_eigen_vector(dot_matrix, variance_thresh):
    ...
    eigen_val = pd.Series(
        eigen_val, index=['PC_' + str(i + 1) for i in range(eigen_val.shape[0])]
    )
...
def feature_pca_analysis(feature_df, feature_importance, variance_thresh=0.95):
    ...
    repeated_importance_array = np.tile(
        feature_importance['mean'].values, len(eigen_vec.columns)
    )
    ...
    feature_pca_rank = (
        (eigen_val * eigen_vec).abs().sum(axis=1).rank(ascending=False)
    )  # Sum of absolute values across all eigen vectors
    corr_dict['Weighted_Kendall_Rank'] = get_pca_rank_weighted_kendall_tau(
        feature_importance['mean'].values, feature_pca_rank.values
    )
    return corr_dict
