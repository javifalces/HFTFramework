...
class GAParameterTuningFunction(GAParameterTuning):
    ...
    def __init__(self, ga_configuration: GAConfiguration, algorithm: AlgorithmEnum, parameters_base: dict,
                 parameters_min: dict, parameters_max: dict, max_simultaneous: int,
                 initial_param_dict_list: list = []):
        super().__init__(ga_configuration, algorithm, parameters_base, parameters_min, parameters_max,
                         max_simultaneous, initial_param_dict_list)
        self.function=optimization_function

        self.population_df = pd.DataFrame(
            columns=['param_dict', 'score']
        )
...
class MockAlgorithm(Algorithm):
    def __init__(self, algorithm_info: str, parameters: dict=DEFAULT_PARAMETERS) -> None:
...
class TestParameterTuning(unittest.TestCase):
    ...
    def test_optimization(self):
        ...
        best_param_dict,population_df_out = algorithm.parameter_tuning()
        self.assertAlmostEqual(best_param_dict['x'],0.0 ,delta= 0.1)
        self.assertAlmostEqual(best_param_dict['y'],0.0 , delta=0.1)
